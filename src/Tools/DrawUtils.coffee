...
drawBlob = (x, y, colour, layer, list=[], brushWidth=1, draw=true) ->
	...
	if draw
		for n in [0..~~(Math.PI * brushWidth)+1]
			...
			dx = Math.round(r * Math.cos(2 * Math.PI * t))
			dy = Math.round(r * Math.sin(2 * Math.PI * t))
			list.push layer.setPixel(x + dx, y + dy, colour)
	else
		for n in [0..~~(Math.PI * brushWidth)+1]
			t = n / ~~(Math.PI * brushWidth+1)
...
drawThickLine = (x, y, dx, dy, colour, layer, list=[], brushWidth=1) ->
	...
	if isFinite gradient
		...
		if samplingFactor > 0
			for x1 in [0..samplingFactor*dx]
				dx1 = x1/samplingFactor
				drawBlob(Math.round(x + dx1), Math.round(y + dx1*gradient), colour, layer, list=list, brushWidth=brushWidth, draw=yes)
		else
			for x1 in [0..dx]
				drawBlob(x + x1, y, colour, layer, list=list, brushWidth=brushWidth, draw=yes)
	else
		for y1 in [0..dy]
			drawBlob(x, y + y1, colour, layer, list=list, brushWidth=brushWidth, draw=yes)
	return list
